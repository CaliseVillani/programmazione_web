...
//############################################## CONTROLLER PER LA PAGINE DI BIOLOGIA
.controller('BiologiaCtrl', function($scope, AuthService, $ionicPopup, $state) {
  $scope.logout = function() {
    AuthService.logout();
    $state.go('outsidehome');
  };
...
// Mostra carrriera dello studente
  $scope.ShowCarriera = function() {
    ...
    function success(currentaccount){
      ...
      }if(currentaccount.data.msg=='matematica'){
        $state.go('carrieramatematica')
      }if(currentaccount.data.msg=='biologia'){
        $state.go('carrierabiologia')
      }
    }
    function error(currentaccount){
      var alertPopup = $ionicPopup.alert({
...
//############################################## CONTROLLER PER LA CARRIERA DI BIOLOGIA
.controller('CarrieraBiologiaCtrl', function($scope, AuthService, $ionicPopup, $state,$http) {
  ...
  function Main($scope) {
    ...
    $scope.carriera.msg.molecular_biology;
    $scope.carriera.msg.analisi;
    $scope.carriera.msg.zoologia;
    $scope.carriera.msg.botanica;
  }
...
//############################################## CONTROLLER PER LA CARRIERA DI MATEMATICA
.controller('CarrieraMatematicaCtrl', function($scope, AuthService, $ionicPopup, $state,$http) {
  $http.get('/pullCarriera').then(success, error);
  function success(carriera){
    $scope.carriera = carriera.data;
  }
  ...
  function Main($scope) {
    ...
    $scope.carriera.msg.ricerca_operativa;
    $scope.carriera.msg.analisi;
  }
  $scope.logout = function() {
    AuthService.logout();
